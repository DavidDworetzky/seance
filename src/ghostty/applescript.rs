...
mod tests {
    ...
    fn test_split_window_direction_uses_command_not_initial_input() {
        ...
        assert!(
            script.contains(r#"set command of cfg to "codex --prompt \"$(cat /tmp/prompt.md)\"""#)
        );
        assert!(!script.contains("set initial input of cfg"));
    }

    #[test]
    fn test_split_window_direction_does_not_inject_codex_into_cloned_claude_surface() {
        let script = split_window_direction(
            "win-1",
            "right",
            Some("codex -m gpt-5.4 -c model_reasoning_effort=\"high\"\n"),
        );

        assert!(script.contains(
            r#"set command of cfg to "codex -m gpt-5.4 -c model_reasoning_effort=\"high\"""#
        ));
        assert!(script.contains("split targetTerminal direction right with configuration cfg"));
        assert!(!script.contains("set initial input of cfg"));
